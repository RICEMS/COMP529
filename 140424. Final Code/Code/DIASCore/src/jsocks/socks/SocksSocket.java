...
public class SocksSocket extends Socket{
   ...
   public SocksSocket(Proxy p,String host,int port)
      ...
      if(proxy.isDirect(host)){
         remoteIP = NameResolver.getNameResolver().getByName(host);
         doDirect();
      }
      else
   ...
   public InetAddress getInetAddress(){
      if(remoteIP == null){
	 try{
	   remoteIP = NameResolver.getNameResolver().getByName(remoteHost);
   ...
   public InetAddress getLocalAddress(){
      if(localIP == null){
	 try{
	    localIP = NameResolver.getNameResolver().getByName(localHost);
	 }catch(UnknownHostException e){
	   return null;
	 }
      }
