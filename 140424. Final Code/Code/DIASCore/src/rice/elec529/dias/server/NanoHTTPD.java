...
public class NanoHTTPD
{
	 ...
	 */
	public NanoHTTPD( int port, File wwwroot, LogInterface log )
	{
		myOut = log;
		myTcpPort = port;
		...
		{
			myOut.log("NanoHTTPD Couldn't start server:\n" + e.getMessage());
		}
	...
	{
		myOut = new LogFixture();
		myOut.log( "NanoHTTPD 1.27 (C) 2001,2005-2013 Jarno Elonen and (C) 2010 Konstantinos Togias\n" +
		...
		NanoHTTPD nano = new NanoHTTPD( port, wwwroot, myOut );
		

		myOut.log( "Now serving files in port " + port + " from \"" + wwwroot + "\"" );
		myOut.log( "Hit Enter to stop.\n" );
		...
		private String saveTmpFile(byte[] b, int offset, int len)
		{
			...
			if (len > 0)
			{
				...
				try {
					...
					myOut.log("Nano Error: " + e.getMessage());
				}
			}
			return path;
		}
							   ...
							   boolean allowDirectoryListing )
	{
		...
		{
			uri = "";
			res = new Response( HTTP_OK, MIME_HTML,
				"<html><body>Hello DIAS ELEC 529!<a href=\"" + uri + "\">" +
				uri + "</a></body></html>");
		...
		myOut.log("Nanohttpd " + homeDir + " " + uri);
	...
	protected static LogInterface myOut;
