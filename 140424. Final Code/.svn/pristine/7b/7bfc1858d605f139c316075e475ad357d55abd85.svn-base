...
public class DiasWebView extends WebViewFragment
{
	static int m_port;
	public DiasWebView()
	{		
	...
	public void onActivityCreated (Bundle savedInstanceState)
	{
		...
		Log.i("DiasWebView", "onActivityCreated, setting proxy!");
		WebView wv = this.getWebView();
		if (wv != null)
			setProxy(wv, "localhost", m_port);
		wv.loadUrl("http://google.com");
	}
	
	private static final String LOG_TAG = "DiasWebView";

	public static boolean setProxy(WebView webview, String host, int port)
	{
		String applicationClassName="android.app.Application";
		return setProxy(webview, host, port, applicationClassName);
	}
	        ...
	        for (Object receiverMap : receivers.values()) {
	            for (Object rec : ((ArrayMap) receiverMap).keySet()) {
	                ...
	                if (clazz.getName().contains("ProxyChangeListener")) {
	                    ...
	                    intent.putExtra("proxy", (CharSequence) proxyProperties);
	                    /*********** optional, may be need in future *************/
