...
public class InetRange implements Cloneable{
    ...
    public synchronized boolean contains(String host,boolean attemptResolve){
       ...
       try{
          InetAddress ip = NameResolver.getNameResolver().getByName(host);
          return contains(ip);
    ...
    public synchronized boolean remove(String s){
      Enumeration<Object[]> e = all.elements();
...
/////////////////
    ...
    private boolean checkHostEnding(String host){
       Enumeration<String> e = end_names.elements();
       while(e.hasMoreElements()){
          if(host.endsWith(e.nextElement())) return true;
       }
       return false;
    }
...
class InetRangeResolver implements Runnable{
   ...
   public void run(){
     try{
        if(from == null){
          InetAddress ip = NameResolver.getNameResolver().getByName((String) entry[0]);
          entry[1] = ip;
          Long l = new Long(InetRange.ip2long(ip));
          entry[2] = entry[3] = l;
        }else{
          InetAddress f = NameResolver.getNameResolver().getByName(from);
          InetAddress t = NameResolver.getNameResolver().getByName(to);
